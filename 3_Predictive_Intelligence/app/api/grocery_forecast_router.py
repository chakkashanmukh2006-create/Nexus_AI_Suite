...
def get_kaggle_dataset():
    csv_path = os.path.join(os.path.dirname(__file__), "../../../datasets/grocery_forecast.csv")
    df = pd.read_csv(csv_path)
    df['Date'] = pd.to_datetime(df['Date'], format="mixed", dayfirst=False)
    return df

@router.get("/grocery/options")
...
@router.get("/grocery/forecast")
def get_grocery_forecast(
    ...
    df_agg = df.groupby('Date')['Demand_Volume'].sum().reset_index()
    ...
    historical_values = df_chart['Demand_Volume'].tolist()
    ...
    series = df_train['Demand_Volume'].values
    ...
    # 4. Prophet
    prophet_df = df_train[['Date', 'Demand_Volume']].rename(columns={'Date': 'ds', 'Demand_Volume': 'y'})
